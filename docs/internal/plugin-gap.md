...
| F (refactoring), H1, H3, H6, E4, E5, C6 | ✅ Fixed (0.0.2, 2026-09-05) | The everyday actions and the Refactor menu, all IDE-side so they work with or without the language server. **F:** Extract Variable (`Ctrl+Alt+V`), Introduce Constant (`Ctrl+Alt+C`), Inline Variable (`Ctrl+Alt+N`) and Safe Delete — gated by `lang.refactoringSupport`, which was the single unregistered provider greying out the whole menu. **H1:** Parameter Info (`Ctrl+P`), every overload, active parameter bold, sharing the active-parameter rule with `juxc-lsp`'s `find_enclosing_call`. **H3:** Surround With (`Ctrl+Alt+T`), the Java template set plus `unsafe { }` and `(expr)`/`!(expr)`. **H6:** Complete Current Statement (`Ctrl+Shift+Enter`) and Move Statement Up/Down. **E4:** Type Hierarchy (`Ctrl+H`), three views over `JuxSubtypes`/`JuxTypeIndex`. **E5:** breadcrumbs — which the plugin description had already claimed. **C6:** spellchecking of comments, plain strings and declaration names. Mutation is document text + reformat throughout, never PSI splicing: `JuxElementFactory` can only build an identifier, and tree surgery is where these features acquire corruption bugs. Suite 336 → 395. |
| B1–B4, C4, E1/E6, H2/H5, I1, J1–J4 | Open | Tracked below — stub index, accessor/record PSI depth, injection, type inlay hints, debugger/coverage remain. Extract Method, Change Signature and Move Class are the next refactoring step; override-method-name completion is still open from D3/D4. |
...
**Partly closed in 0.0.2.** Present: **Extract Variable**, **Introduce Constant**,
**Inline Variable**, **Safe Delete** and Rename (in place for locals and
parameters), behind a `lang.refactoringSupport` provider. Still absent: Move
(class/file/package), **Extract Method**, Extract Parameter/Field, Change
Signature, Pull Up/Push Down. Extract Method is the largest remaining one and the
natural next step, since it needs the same expression analysis plus a read/write
flow over the selected statements.
