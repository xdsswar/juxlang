...
> **Status (2026-06-12): DONE.** §18.6 written into `JUX-ASYNC-ADDENDUM-v2.md`
> and implemented: built-in `Stream<T>` (`async T? next()`), `for await`,
> `of`/`from`/`generate`, `mapAsync`/`filterAsync`/`take`/`skip`/`chain`,
> implicit backpressure, E0703/E0704 diagnostics. Async generators and async
> transform lambdas remain deferred (§18.7).
...
> **Status (2026-06-12): DONE.** `JUX-TESTING-ADDENDUM.md` written and
> implemented: `@Test` + the four lifecycle hooks (per-file scope),
> `jux.std.testing` assertions (incl. `assertNear`, the Phase-1
> `assertThrows` ruling), async tests via `block_on`, runtime filtering
> (`jux test <pattern>`), `--release` with always-checked `assert`.
> Deferred per §TS.9: parallel execution, JUnit XML, `@Property`,
> `@Ignore`.
