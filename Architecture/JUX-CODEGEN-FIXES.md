...
- [x] Every Jux string literal in the AST lowers to `"...".to_string()` in emitted Rust.
- [x] No function parameter or return type in emitted Rust uses `&str` for a Jux `String` value.
- [x] The mixed-arm switch test case below compiles without manual coercion.
- [x] All existing examples in `examples/` still compile + run with identical observed output.
...
- [x] `rustfmt --edition=2021 --quiet` is invoked on every emitted `.rs` file between write and `rustc` invocation (`juxc-driver::run_rustfmt`).
- [x] Failure to find `rustfmt` produces a single warning to stderr (first file only) and does NOT fail the build.
- [x] A debug escape hatch exists for inspecting the raw emitter output: the
      `JUX_NO_RUSTFMT=1` environment variable skips formatting. (Implemented as an
      env var rather than a CLI flag to avoid threading a debug-only knob through
      every compile entry point; the effect is identical.)
- [x] Emitted output is idempotent under rustfmt — `rustfmt --check --edition=2021`
      on a generated `.rs` file is a no-op (verified on the examples corpus).
...
- [x] Every emitted `.rs` file begins with the AUTO-GENERATED banner + source path + the eight `#![allow(...)]` attributes listed above.
- [x] Compiling the entire `examples/` directory produces zero `warning:` lines on stderr.
- [x] The banner's source path is the absolute path of the originating `.jux` file (for click-through in editors that hyperlink terminal output).
...
- [x] An interpolated string with zero `${…}` segments lowers to the same form as a plain literal (Fix 1 path), not to a `format!(…)` call.
- [x] An interpolated string with one or more `${…}` segments still lowers to `format!(…)`.
- [x] `cargo clippy --` on the emitted output of `examples/colors.jux` and similar surfaces no `useless_format` warnings.
...
| # | Fix | Priority | Status | Visible Impact |
|---|---|---|---|---|
| 1 | String literals → `String` | High | ✅ done | Eliminates an entire class of match-arm type errors; locks "no `&str` for `String`" rule |
| 2 | Run `rustfmt` on output | High | ✅ done | Generated code looks professional; output is rustfmt-idempotent. `JUX_NO_RUSTFMT=1` skips it for raw-output debugging |
| 3 | `#![allow(...)]` header | High | ✅ done | No spurious Rust warnings reach the user (corpus emits zero `warning:` lines) |
| 4 | `Display` includes payloads | Medium | ✅ done | Spec compliance (§7.7.2); `print($"$enum")` shows `Variant(field: v, …)` |
| 5 | `$"stop"` → `"stop".to_string()` | Low | ✅ done | Minor polish, removes Clippy noise; reuses Fix 1's helper |
| 6 | Inherited interface `default` callable on concrete class (§7.4.3) | High | ✅ done | `obj.defaultMethod()` resolves inherent-first via a forwarding `<Self as Trait<args>>::m(self)` stub; fixes E0599 "trait not in scope" |

**All five fixes are landed and verified** (suite green; sweep of diverse examples
shows zero warnings and rustfmt-idempotent output). None were architectural changes —
each was a small, bounded patch to the codegen.
...
## Fix 6 — inherited interface `default` methods callable on the concrete class (§7.4.3)

**Symptom.** A `default` method on an interface was not callable as an
inherent method on an implementing class that did **not** override it.
Given `interface Holder<T> { default String label() { … } }` and
`class HolderName implements Holder<Object>` (no `label` override),
`holder.label()` (holder: `HolderName`) produced emitted Rust that
failed to compile:

```text
error[E0599]: no method named `label` found for struct `HolderName`
  help: trait `Holder` which provides `label` is implemented but not in scope
```

**Root cause.** A Jux interface lowers to a Rust *trait*, and a `default`
method lowers to a Rust *trait default body* — emitted on `trait Holder`,
not into `impl Holder for HolderName`. Rust resolves `holder.label()`
inherent-methods-FIRST; the class's inherent `impl` carried only the
overridden methods, so `label` missed and rustc demanded the trait be in
scope at the call site — which it wasn't (no `use Holder`). In a
multi-file project the trait lives in a *different module* from the class,
so the call never built. (Single-file programs where the trait happened to
be in the same module masked the bug — `examples/interface_static.jux`
worked only by that accident.)

**Fix.** The backend now emits an **inherent forwarding method** on each
implementing class for every interface `default` method it does not
override, calling the trait default through a *fully-qualified* path so no
`use` is needed:
...
This makes `holder.label()` resolve inherent-first (exactly like an
overridden method) while reusing the trait's default body. The
fully-qualified `<Self as Trait<args>>::method(self, …)` form is what
removes the `use` requirement; the trait path + its generic args are
spelled by the same emitter the `impl Trait for Class` header uses
(`emit_type_as_rust(interface_ty)`), so it always renders the
crate-qualified path.

**Where.** `crates/juxc-backend-rust/src/decls/classes.rs`,
`emit_inherited_default_methods` (modeled on `emit_class_trait_impls`):
rolls up `implements` through the `extends` chain, builds the per-interface
`T → arg` substitution (so a `Holder<Object>` default returning `T`
forwards as `Object`), and skips any default the class overrides or that a
concrete ancestor already inlined as an inherent (those resolve
inherent-first already via `emit_inherited_wrapper_methods`). Static and
abstract interface methods are skipped (no trait-default body to forward
