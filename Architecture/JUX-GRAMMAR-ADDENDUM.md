...
3. **`(expr)` vs `(expr,)` (trailing comma).** A trailing comma in a parenthesized expression is a single-element tuple in some other languages. In Jux v1 it is reserved syntax (`E0302` "single-element tuples are not in v1; remove the trailing comma or convert to a record").
