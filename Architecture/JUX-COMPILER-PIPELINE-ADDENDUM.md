...
| `yield expr`                                | Phase 1: `__jux_co.yield_(expr).await` inside an `async move` body that `JuxGenerator` polls (Missing-defs §M.2.5) |
...
- **`yield` generators.** Rust's coroutine syntax is unstable. Phase 1 borrows the state machine rustc already builds for an `async` block instead (Missing-defs §M.2.5). The user-visible behavior is identical.
