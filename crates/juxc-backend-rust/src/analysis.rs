...
pub(crate) use juxc_ast::visit::{for_each_expr, for_each_expr_in};
...
/// (§M.13.2, ERRATA E90): the names that appear, bare, as the initializer of a
/// `ref` declaration.
///
...
/// by its class, so `ref int s = p.counter;` still copies (ERRATA E90).
...
pub(crate) fn collect_mutating_calls(e: &Expr, out: &mut HashSet<String>, user_mut: &HashSet<String>) {
    match e {
        ...
        Expr::Throw(inner, _) => collect_mutating_calls(inner, out, user_mut),
...
fn expr_contains_await(e: &Expr) -> bool {
    match e {
        ...
        Expr::Throw(inner, _) => expr_contains_await(inner),
        Expr::Call(c) => {
