...
| H4-4 | `ref` FIELDS (`public ref String x`) — parse + flag done, lowering (storage wrap + owner-routed read/write) pending | A (in progress) | NEXT: wrap field storage, `.borrow().clone()` reads, `*….borrow_mut()` writes through inline + wrapper owners, ctor-init wrap. |
