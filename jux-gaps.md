...
| C6 | Collections pass **by value** — a callee's `push` is invisible to the caller (`probes/probe_vec_param.jux` prints `len after call: 0`) | B | ◐ **Specced (2026-06-13).** Decision: **Java reference semantics** — collections are shared handles (`Rc<RefCell<…>>`, like classes §CR). Spec `JUX-COLLECTIONS-ADDENDUM.md` §V. Implementation in progress (B1). |
...
| J5 | 2D/multi-dim array TYPE syntax `int[][]` doesn't parse (`new int[3][3]` parses but the type annotation doesn't) | B | ✅ **Fixed (2026-06-13).** `ArrayShape` is now `{ dims: Vec<ArrayDim> }` (outermost-first); the parser loops array suffixes, so `int[][]`, `int[][][]`, `int[3][4]`, and mixed `int[3][]` all parse. `NewArrayExpr` gained `inner_sizes` for `new int[3][4]`. Lowering nests per dim — dynamic → `Vec<…>`, fixed → `[…; N]`, mixed composes (`int[3][]` → `[Vec<isize>; 3]`). Indexing peels one dim (`ArrayShape::peeled()`). Spec §5.5 + grammar §A.2.7. `examples/multidim_arrays.jux`. |
...
| N3 | ~~Expression-position `++`/`--` (`print(x++)`, `arr[i++]` as a value) not supported — only statement + for-update positions~~ | C | ✅ **Fixed (2026-06-13).** New `Expr::IncDec { target, is_inc, is_prefix }`. The EXPRESSION grammar (parse_unary prefix / parse_postfix postfix) builds the value node; the STATEMENT + for-update paths still desugar a discarded `++`/`--` to `place += 1` (unwrapping the IncDec node when it is the whole statement, so emitted Rust stays a clean `+= 1`). Tycheck: result = operand type, place must be assignable + numeric (E0200). Backend lowers to a value-returning block reusing `emit_assign` for the store (wrapper/ref/static/operator[]= all correct), hoisting a side-effecting index/receiver into a temp for single-eval (`arr[next()]++` runs `next()` once); post yields OLD (`__jux_t`), pre yields NEW. Spec: §A grammar un-deferred + new Appendix A.6 (value semantics + Rust lowering) + precedence table lvls 18/19. `examples/inc_dec_expr.jux` + `bin/jux/tests/inc_dec_expr.rs`; parser/tycheck/backend unit tests. Statement-form `increment_decrement` e2e unregressed. |
